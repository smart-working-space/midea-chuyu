...
(function(mdSmartios) {
     ...
     *                   -2 : 失败(无文件)
     */
    mdSmartios.bridge.b2bAction = function(param, callback, callbackFail) {
        ...
        var commandId = mdSmartios.bridge.po._execObjcMethod('b2baction', p);

        return commandId;
    };

    mdSmartios.bridge.setAndroidGoBack = function() {
        var url = location.href;
        var urlLength = url.length;
        if (url != '' && urlLength >= 2) {
            last2Char = url.substring(urlLength - 2, urlLength);
            if (url.indexOf("index") != -1 || last2Char === '#/') {
     ...
     *                   -2 : 失败(无文件)
     */
    mdSmartios.bridge.getDeviceList = function(callback, callbackFail) {
        var param = {};
        param.cammandId = Math.floor(Math.random() * 1000);
        var commandIds = param.cammandId;
        var p = JSON.stringify(param);
        ...
        var commandId = mdSmartios.bridge.po._execObjcMethod('getDeviceList', p);
        ...
        if (jsonResult.errCode !== undefined && jsonResult.errMessage == 'TimeOut') {
            if (jsonResult.errCode == -1) {
                ...
                if (typeof cbff == "function") {
                    ...
                    $(".timeout").html("控制超时");
                    $(".timeout").show().animate({ "opacity": 1 }, 10);
                    $(".timeout").animate({ "opacity": 0 }, 1000);
                    $(".myLoadingWrapper").css({
                        "display": "none"
                    });
                }
    ...
    /*
     *指定命令的回复电文(Android调用：回复getterVal)
     *@param (int) retObjcValue 命令id
     *       (String) result 回复
     *                result.messageBody 回复电文
     *                result.errCode 错误码
